...
import { AIChatMessage, Transaction, Contact, Campaign } from '../types';
import { generateFinancialAdvice } from '../services/geminiService';
import { Send, User, Sparkles, Trash2, ExternalLink, X, Minus } from 'lucide-react';
...
const AIChat: React.FC<AIChatProps> = ({ isOpen, onClose, transactions, contacts, campaigns, history, onUpdateHistory, onNavigateToCampaign }) => {
  ...
  useEffect(() => {
    if (history.length === 0) {
      onUpdateHistory([{
        id: 'welcome',
        role: 'model',
        text: "Hello! I'm your Ladly Media assistant. I have full context of your ledger, contacts, and active campaigns. How can I help you today?",
        timestamp: new Date()
      }]);
    }
  }, []);
  ...
  const handleSend = async () => {
    ...
    const responseText = await generateFinancialAdvice(input, transactions, contacts, campaigns);
    ...
    `}>
      <div className="p-5 border-b border-gray-100 dark:border-slate-700 bg-sidebar text-white flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
              <h2 className="font-black text-sm uppercase tracking-widest">Ladly AI</h2>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Intelligence by Ladly Media</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-bg-page/50">
        ...
        {isLoading && (
          <div className="flex gap-3">
            ...
            <div className="bg-bg-card border border-gray-100 dark:border-slate-800 px-4 py-3 rounded-2xl rounded-tl-none shadow-sm flex items-center gap-1.5">
      ...
      <div className="p-4 border-t border-gray-100 dark:border-slate-700 bg-bg-card shrink-0">
        ...
        <div className="mt-4 flex flex-col items-center gap-1 opacity-60">
           ...
           <p className="text-center text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">All rights reserved Ladly Media FZ LLC 2025</p>
        </div>
      </div>
    </div>
  );
